<?php
require_once "common.php";
require_once "connection.php";

$session = new Session();
$db = new Database();

if (!$session->get('user') && !$session->get("isAdmin")) {
    header("Location: 403.php");
}

if (!isPost()) {
    header("Location: 403.php");
}

$data = getBody();

if(!isset($data)) {
    header("Location: 403.php");
}

$userID = $session->get("user");
$contactID = $data["id"];

if (!isset($contactID) || !filter_var($contactID, FILTER_VALIDATE_INT)) {
    header("Location: 404.php");
}

$contactSQL = "UPDATE contact SET subject = :subject, message = :message WHERE id = :id";
$statement = $db->prepare($contactSQL);
$statement->bindParam(":id", $contactID, PDO::PARAM_INT);
$statement->bindValue(":subject", $data["subject"]);
$statement->bindValue(":message", $data["message"]);
$statement->execute();
header("Location: /admin.php#contact");

?>